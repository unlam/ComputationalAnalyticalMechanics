...
El teorema de la energía que establece que la suma de las energías cinéticas y potenciales no varía durante un movimiento da solo \emph{una} ecuación, cuando es evidente que describir el movimiento de solo una partícula requiere de \emph{tres} (las dimensiones espaciales), incrementado tal número si el sistema está compuesto de más partículas.
...
Así como en la ecuación \eqref{Lanczos17.2} se expresó el trabajo infinitesimal en función de las coordenadas de un sistema cartesiano, puede hacerse lo mismo con coordenadas generalizadas
...
\begin{equation}\label{Lanczos22.7}
    ...
    \tag{Lanczos 22.7}
\end{equation}
Y si bien un punto \emph{silla} de \(V\) también cumpliría tal condición, cada uno de los puntos que la cumplen se los considera excepcionales; se dice que la función presenta un valor \emph{estacionario} allí.
...
\begin{quote}
``Cualquier sistema de fuerzas está en equilibrio si se agregan a las fuerzas \emph{ejercidas} la de inercia''.
\end{quote}
