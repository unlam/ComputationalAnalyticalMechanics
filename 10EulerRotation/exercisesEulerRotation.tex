...
\begin{enumerate}
	...
	\begin{minipage}[t][5.5cm]{0.65\textwidth}
		...
		\textbf{Help:} Do not forget that in G a ``centrifugal'' acceleration is perceived, \(\vec{a}_{c} = \dfrac{|\alpha_c|}{d_{AG}} \hat{x}\), when expressing Newton's second law equations. It will be up to the bearing at B to provide a reaction in \(\hat{x}\) to exert the centripetal force that keeps the cylinder on its circular path.
	\end{minipage}
	...
	\begin{tasks}
		...
		Result: \(
		\left[\begin{matrix}A_{y}\\A_{z}\\B_{x}\\B_{y}\\B_{z}\end{matrix}\right] = \left[\begin{matrix}-17.4\\33.4\\134.0\\17.4\\33.4\end{matrix}\right]
		\)
	\end{tasks}
